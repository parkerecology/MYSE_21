...
###############  NABat Stan COLEXT models 2015-2021 ###################
...
# }
# 
# LL<-LongLatToUTM(sc$Longitude,sc$Latitude,17)
# 
# sc<-cbind(sc[,-c(6,7)],LL[,-c(1)])
...
# pesu_umf@siteCovs<-
#   pesu_umf@siteCovs%>%
#   mutate(across(where(is.numeric) & !c(sc.X,sc.Y,sc.id),scale))

pesu_umf@siteCovs<-
pesu_umf@siteCovs%>%
mutate(across(where(is.numeric) & !c(sc.Latitude,sc.Longitude,sc.id),scale))
...
umf_hng2 <- pesu_umf[,4]#cut out one year for testing, 

pesu_umf1<-unmarkedFrameOccu(y=umf_hng2@y,
                             siteCovs = umf_hng2@siteCovs,
                             obsCovs = umf_hng2@obsCovs)

summary(pesu_umf1)
plot(pesu_umf1)


pesu_umf2<-pesu_umf1[which(rowSums(is.na(pesu_umf1@y)) != ncol(pesu_umf1@y)),]

t1<-occu(~tavg+water ~sc.canopy,data =pesu_umf2 )
summary(t1)

b_t1<-stan_occu(~tavg+water ~sc.canopy,pesu_umf2, chains=3)
b_t1
...
data_sf <- st_as_sf(sc, coords = c("Longitude", "Latitude"),
                    # Change to your CRS
...
d <- gDistance(coords, byid=T)
min.d <- apply(d, 1, function(x) order(x, decreasing=F)[2])

newdata <- cbind(coords, coords[min.d,], apply(d, 1, function(x) sort(x, decreasing=F)[2]))

pesu_umf2@siteCovs <- st_as_sf(pesu_umf2@siteCovs, coords = c("sc.Longitude", "sc.Latitude"),
                    # Change to your CRS
                    crs = "+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs")

pesu_umf2@siteCovs$geometry<-st_transform(pesu_umf2@siteCovs$geometry,32617)

st_is_longlat(pesu_umf2@siteCovs)

pesu_umf2@siteCovs$geometry


xy_c<-do.call(rbind, st_geometry(pesu_umf2@siteCovs$geometry)) %>% 
  as_tibble() %>% setNames(c("X","Y"))

pesu_umf2@siteCovs<-cbind(pesu_umf2@siteCovs, xy_c)

pesu_umf2@siteCovs$geometry<-as.vector(pesu_umf2@siteCovs$geometry)

form <- ~tavg+water ~sc.canopy + RSR(X,Y,threshold=0.5)

options(mc.cores=3)
fit_ubms <- stan_occu(form, pesu_umf2, chains=3)

plot_spatial(fit_ubms,"state")

#choose a threshold value for RSR 

#convert lat/long into projected
library(sp)
library(rgdal)
...
site_covs<-merge(s21[,c(2,6,7)],sc,by=c("site_code"))

coords<-site_covs[,c(1,2,3)]
...
pesu_umf2@siteCovs<-LongLatToUTM(pesu_umf2@siteCovs$sc.Longitude,pesu_umf2@siteCovs$sc.Latitude,17)

site_covs<-cbind(p_coords,site_covs)
...
ggplot(data=site_covs, aes(x=X, y=Y)) +
  geom_tile(aes(fill=impervious)) +
  geom_point(size=0.5) +
...
with(site_covs,RSR(X,Y,threshold=2),plot_site=15)
...
m.pesu.global<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tavg+j_date+structure+tree_canopy+canopy+water+elev+dis_2_clutter, data=pesu_umf)
...
#null 
m.pesu.null<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~1, data=pesu_umf)
summary(m.pesu.null)      

#J-date 
m.pesu.jdate<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~scale(j_date), data=pesu_umf)
summary(m.pesu.jdate)  

#J-date2 
m.pesu.jdate2<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~scale(j_date)+I(scale(j_date)^2), data=pesu_umf)
summary(m.pesu.jdate2)

#J-date3 
m.pesu.jdate3<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~scale(j_date)+I(scale(j_date)^2)+I(scale(j_date)^3), data=pesu_umf)
summary(m.pesu.jdate3)


#tmin 
m.pesu.tmin<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tmin, data=pesu_umf)
summary(m.pesu.tmin)      

#tmin2 
m.pesu.tmin2<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tmin+I(tmin^2), data=pesu_umf)
summary(m.pesu.tmin2) 

#tavg 
m.pesu.tavg<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tavg, data=pesu_umf)
summary(m.pesu.tavg) 

#tavg 2
m.pesu.tavg2<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tavg+I(tavg^2), data=pesu_umf)
summary(m.pesu.tavg2)

#tmax 
m.pesu.tmax<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tmax, data=pesu_umf)
summary(m.pesu.tmax) 

#tmax2 
m.pesu.tmax2<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tmax+I(tmax^2), data=pesu_umf)
summary(m.pesu.tmax2) 

#structure 
m.pesu.structure<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~structure, data=pesu_umf)
summary(m.pesu.structure) 

#distance to clutter
m.pesu.dis_2_clutter<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~dis_2_clutter, data=pesu_umf)
summary(m.pesu.dis_2_clutter) 

#canopy catagory
m.pesu.canopy<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~canopy, data=pesu_umf)
summary(m.pesu.canopy) 

#canopy continous
m.pesu.tree_canopy<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tree_canopy, data=pesu_umf)
summary(m.pesu.tree_canopy) 

#canopy continous2 
m.pesu.tree_canopy2<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~scale(tree_canopy)+I(scale(tree_canopy)^2), data=pesu_umf)
summary(m.pesu.tree_canopy2) 

#Water 
m.pesu.water<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water, data=pesu_umf)
...
#water + jdate
m.pesu.water.jdate<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + j_date, data=pesu_umf)
summary(m.pesu.water.jdate) 

#water + structure
m.pesu.water.structure<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + structure, data=pesu_umf)
summary(m.pesu.water.structure) 

#water + tmin
m.pesu.water.tmin<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + tmin, data=pesu_umf)
summary(m.pesu.water.tmin) 

#water + tavg
m.pesu.water.tavg<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + tavg, data=pesu_umf)
summary(m.pesu.water.tavg)

#water + tmax
m.pesu.water.tmax<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + tmax, data=pesu_umf)
summary(m.pesu.water.tmax)

#water + canopy
m.pesu.water.canopy<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + canopy, data=pesu_umf)
summary(m.pesu.water.canopy)

#water + tree canopy 
m.pesu.water.tree_canopy<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + tree_canopy, data=pesu_umf)
summary(m.pesu.water.tree_canopy)

#water + distance to clutter
m.pesu.water.dis_2_clutter<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + dis_2_clutter, data=pesu_umf)
summary(m.pesu.water.dis_2_clutter)

#structure + tmax
m.pesu.structure.tmax<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~structure + tmax, data=pesu_umf)
summary(m.pesu.structure.tmax)

#structure + jdate
m.pesu.structure.jdate<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~structure + j_date, data=pesu_umf)
summary(m.pesu.structure.jdate)

#canopy + jdate
m.pesu.canopy.jdate<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~canopy + j_date, data=pesu_umf)
...
#water +/x structure
m.pesu.water_structure<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water*structure, data=pesu_umf)
summary(m.pesu.water_structure) 

#water +/x tree_canopy
m.pesu.water_tree_canopy<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water*tree_canopy, data=pesu_umf)
summary(m.pesu.water_tree_canopy) 

#water +/x structure + tmax
m.pesu.water_structure.tmax<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water*structure+tmax, data=pesu_umf)
...
m.epfu.global<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tmin+tavg+tmax+j_date+structure+tree_canopy+canopy+water+elev+dis_2_clutter+impervious, data=epfu_umf)
...
#null 
m.epfu.null<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~1, data=epfu_umf)
summary(m.epfu.null)      

#J-date 
m.epfu.jdate<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~j_date, data=epfu_umf)
summary(m.epfu.jdate)  

#J-date2 (wont run)  
m.epfu.jdate2<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~j_date+I(j_date^2), data=epfu_umf)
summary(m.epfu.jdate2)

#tmin 
m.epfu.tmin<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tmin, data=epfu_umf)
summary(m.epfu.tmin)      

#tmin2 
m.epfu.tmin2<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tmin+I(tmin^2), data=epfu_umf)
summary(m.epfu.tmin2) 

#tavg 
m.epfu.tavg<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tavg, data=epfu_umf)
summary(m.epfu.tavg) 

#tavg 2
m.epfu.tavg2<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tavg+I(tavg^2), data=epfu_umf)
summary(m.epfu.tavg2)

#tmax 
m.epfu.tmax<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tmax, data=epfu_umf)
summary(m.epfu.tmax) 

#tmax2 
m.epfu.tmax2<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tmax+I(tmax^2), data=epfu_umf)
summary(m.epfu.tmax2) 

#structure 
m.epfu.structure<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~structure, data=epfu_umf)
summary(m.epfu.structure) 

#distance to clutter
m.epfu.dis_2_clutter<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~dis_2_clutter, data=epfu_umf)
summary(m.epfu.dis_2_clutter) 

#canopy catagory
m.epfu.canopy<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~canopy, data=epfu_umf)
summary(m.epfu.canopy) 

#canopy continous
m.epfu.tree_canopy<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tree_canopy, data=epfu_umf)
summary(m.epfu.tree_canopy) 

#canopy continous2 
m.epfu.tree_canopy2<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tree_canopy+I(tree_canopy^2), data=epfu_umf)
summary(m.epfu.tree_canopy2) 

#Water 
m.epfu.water<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water, data=epfu_umf)
...
#water + jdate
m.epfu.water.jdate<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + j_date, data=epfu_umf)
summary(m.epfu.water.jdate) 

#water + structure
m.epfu.water.structure<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + structure, data=epfu_umf)
summary(m.epfu.water.structure) 

#water + tmin
m.epfu.water.tmin<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + tmin, data=epfu_umf)
summary(m.epfu.water.tmin) 

#water + tavg
m.epfu.water.tavg<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + tavg, data=epfu_umf)
summary(m.epfu.water.tavg)

#water + tmax
m.epfu.water.tmax<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + tmax, data=epfu_umf)
summary(m.epfu.water.tmax)

#water + canopy
m.epfu.water.canopy<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + canopy, data=epfu_umf)
summary(m.epfu.water.canopy)

#water + tree canopy 
m.epfu.water.tree_canopy<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + tree_canopy, data=epfu_umf)
summary(m.epfu.water.tree_canopy)

#water + distance to clutter
m.epfu.water.dis_2_clutter<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + dis_2_clutter, data=epfu_umf)
summary(m.epfu.water.dis_2_clutter)

#structure + tmax
m.epfu.structure.tmax<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~structure + tmax, data=epfu_umf)
summary(m.epfu.structure.tmax)

#structure + jdate
m.epfu.structure.jdate<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~structure + j_date, data=epfu_umf)
summary(m.epfu.structure.jdate)


#canopy + jdate
m.epfu.canopy.jdate<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~canopy + j_date, data=epfu_umf)
...
#water +/x structure
m.epfu.water_structure<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water*structure, data=epfu_umf)
summary(m.epfu.water_structure) 

#water +/x tree_canopy
m.epfu.water_tree_canopy<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water*tree_canopy, data=epfu_umf)
summary(m.epfu.water_tree_canopy) 

#water +/x structure + tmax
m.epfu.water_structure.tmax<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water*structure+tmax, data=epfu_umf)
...
m.myse.global<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tavg+j_date+structure+tree_canopy+water+elev+dis_2_clutter, data=myse_umf)
...
#null 
m.myse.null<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~1, data=myse_umf)
summary(m.myse.null)      

#J-date 
m.myse.jdate<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~j_date, data=myse_umf)
summary(m.myse.jdate)  

#J-date2 (wont run)  
m.myse.jdate2<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~j_date+I(j_date^2), data=myse_umf)
summary(m.myse.jdate2)

#tmin 
m.myse.tmin<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tmin, data=myse_umf)
summary(m.myse.tmin)      

#tmin2 
m.myse.tmin2<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tmin+I(tmin^2), data=myse_umf)
summary(m.myse.tmin2) 

#tavg 
m.myse.tavg<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tavg, data=myse_umf)
summary(m.myse.tavg) 

#tavg 2
m.myse.tavg2<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tavg+I(tavg^2), data=myse_umf)
summary(m.myse.tavg2)

#tmax 
m.myse.tmax<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tmax, data=myse_umf)
summary(m.myse.tmax) 

#tmax2 
m.myse.tmax2<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tmax+I(tmax^2), data=myse_umf)
summary(m.myse.tmax2) 

#structure 
m.myse.structure<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~structure, data=myse_umf)
summary(m.myse.structure) 

#distance to clutter
m.myse.dis_2_clutter<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~dis_2_clutter, data=myse_umf)
summary(m.myse.dis_2_clutter) 

#canopy catagory
m.myse.canopy<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~canopy, data=myse_umf)
summary(m.myse.canopy) 

#PSI canopy catagory 
m.myse.psicanopy<-stan_colext(psiformula = ~sc.canopy, gammaformula = ~1, epsilonformula = ~1, pformula = ~1, data=myse_umf)
summary(m.myse.psicanopy) 


#canopy continous
m.myse.tree_canopy<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tree_canopy, data=myse_umf)
summary(m.myse.tree_canopy) 

#canopy continous2 
m.myse.tree_canopy2<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tree_canopy+I(tree_canopy^2), data=myse_umf)
summary(m.myse.tree_canopy2) 

#Water 
m.myse.water<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water, data=myse_umf)
...
#water + jdate
m.myse.water.jdate<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + j_date, data=myse_umf)
summary(m.myse.water.jdate) 

#water + structure
m.myse.water.structure<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + structure, data=myse_umf)
summary(m.myse.water.structure) 

#water + tmin
m.myse.water.tmin<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + tmin, data=myse_umf)
summary(m.myse.water.tmin) 

#water + tavg
m.myse.water.tavg<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + tavg, data=myse_umf)
summary(m.myse.water.tavg)

#water + tmax
m.myse.water.tmax<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + tmax, data=myse_umf)
summary(m.myse.water.tmax)

#water + canopy
m.myse.water.canopy<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + canopy, data=myse_umf)
summary(m.myse.water.canopy)

#water + tree canopy 
m.myse.water.tree_canopy<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + tree_canopy, data=myse_umf)
summary(m.myse.water.tree_canopy)

#water + distance to clutter
m.myse.water.dis_2_clutter<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water + dis_2_clutter, data=myse_umf)
summary(m.myse.water.dis_2_clutter)

#structure + tmax
m.myse.structure.tmax<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~structure + tmax, data=myse_umf)
summary(m.myse.structure.tmax)

#structure + jdate
m.myse.structure.jdate<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~structure + j_date, data=myse_umf)
summary(m.myse.structure.jdate)


#canopy + jdate
m.myse.canopy.jdate<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~canopy + j_date, data=myse_umf)
...
#water +/x structure
m.myse.water_structure<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water*structure, data=myse_umf)
summary(m.myse.water_structure) 

#water +/x tree_canopy
m.myse.water_tree_canopy<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water*tree_canopy, data=myse_umf)
summary(m.myse.water_tree_canopy) 

#water +/x structure + tmax
m.myse.water_structure.tmax<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~water*structure+tmax, data=myse_umf)
...
library(MuMIn)
# 
# m.pesu.global<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tavg+j_date+structure+tree_canopy+water+elev+dis_2_clutter, data=pesu_umf)
# summary(m.pesu.global)
# 
# m.epfu.global<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tavg+j_date+structure+tree_canopy+water+elev+dis_2_clutter, data=epfu_umf)
# summary(m.epfu.global)
# 
# m.myse.global<-stan_colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tavg+j_date+structure+tree_canopy+water+elev+dis_2_clutter, data=myse_umf)
# summary(m.myse.global)
