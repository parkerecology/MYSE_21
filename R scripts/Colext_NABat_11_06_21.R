...
library(ggplot2)
library(ggpubr)
library(beepr)#to add beeps when script completes
...
# drop vars not needed (just rename for now)
sc<-s21
...
m.pesu.global<-colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~tavg+j_date+I(j_date^2)+structure+tree_canopy+canopy+water+elev+dis_2_clutter, data=pesu_umf)
...
#J-date3 
m.pesu.jdate3<-colext(psiformula = ~1, gammaformula = ~1, epsilonformula = ~1, pformula = ~j_date+I(j_date^2)+I(j_date^3), data=pesu_umf)
...
psi.pesu.models<-list("psi(.)gam(.)eps(.)p(average temp + water)"=psi.pesu.null,#
                    ...
                    "psi(elev)gam(.)eps(.)p(average temp + water)"=psi.pesu.elev,#
                    "psi(elev2)gam(.)eps(.)p(average temp + water)"=psi.pesu.elev2,#
                    "psi(water)gam(.)eps(.)p(average temp + water)"=psi.pesu.water,#
                    "psi(canopy)gam(.)eps(.)p(average temp + water)"=psi.pesu.canopy,#
                    "psi(structure)gam(.)eps(.)p(average temp + water)"=psi.pesu.structure,#
                    ...
                    "psi(elev + impervious)gam(.)eps(.)p(average temp + water)"=psi.pesu.elev_impervious,#
                    "psi(elev + water)gam(.)eps(.)p(average temp + water)"=psi.pesu.elev_water,#
                    "psi(elev + tree_canopy)gam(.)eps(.)p(average temp + water)"=psi.pesu.elev_tree_canopy,#
                    "psi(water + tree canopy)gam(.)eps(.)p(average temp + water)"=psi.pesu.water_tree_canopy#
)

#nothing better than the null 
aictab(cand.set=psi.pesu.models,second.ord = FALSE, c.hat=2.23)
...
psi.pesu.500m.models<-list("psi(.)gam(.)eps(.)p(average temp + water)"=psi.pesu.null,#
                      ...
                      "psi(Canopy 0.5k median)gam(.)eps(.)p(average temp + water)"=psi.pesu.canopy500m_median
)

#nothing better than the null 
aictab(cand.set=psi.pesu.500m.models,second.ord = FALSE, c.hat=2.23)
...
psi.pesu.1km.models<-list("psi(.)gam(.)eps(.)p(average temp + water)"=psi.pesu.null,#
                          "psi(landform 1km)gam(.)eps(.)p(average temp + water)"=psi.pesu.landform1k,#
                           ...
                           "psi(Canopy 1km median)gam(.)eps(.)p(average temp + water)"=psi.pesu.canopy1km_median
)

#nothing better than the null 
aictab(cand.set=psi.pesu.1km.models,second.ord = FALSE, c.hat=2.23)
...
psi.pesu.2500m.models<-list("psi(.)gam(.)eps(.)p(average temp + water)"=psi.pesu.null,#
                          ...
                          "psi(Canopy 2500m median)gam(.)eps(.)p(average temp + water)"=psi.pesu.canopy2500m_median
)
...
summary(psi.pesu.urban5km_mean)#SE 3 times estimate
summary(psi.pesu.urban5km_median)#huge SE, 25 times estimate, mean looks better
summary(psi.pesu.urban5km_mean2)#SE 3 times estimate
